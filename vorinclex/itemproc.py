...
for g in items:
	...
	try:
		...
		uniLog("Error opening file "+str(workingitem)+'hist.json')
		continue
	...
	#print(displayflux+' Daily ISK flux for : '+ activeitems[str(g)]['ItemName'])

	#UPDATE ACTIVEITEMS LIST WITH VOLUME
	activeitems[str(workingitem)].update({'DailyISKFlux' : dailyISKflux})
	activeitems[str(workingitem)].update({'DailyVol' : avgVol})

	#Create array with daily isk volume as first index item, for easy sorting
	ranklist.append([dailyISKflux,workingitem,activeitems[str(g)]['ItemName'],avgVol])
